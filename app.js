...
/* =========================================================
   STATE
   ========================================================= */
GEN.push(...(typeof MochiRepair!=='undefined'?MochiRepair.generators:[]), ...MochiBank.generators, ...MochiChallenges.generators, ...(typeof MochiTransfer!=='undefined'?MochiTransfer.generators:[]));
...
const MOCHI_AVATAR = 'mochi-flat-avatar.svg?v=6.2.1';
...
function renderQuestion(customQuestion){
  ...
  if(!customQuestion){
    const recent=new Set(learning().attempts.slice(-40).map(a=>a.question));
...
$('resetBtn').onclick = ()=>{
  ...
  if(window.MochiEntrance)S.entrance=MochiEntrance.fresh();
  if(window.MochiSciencePath)S.sciencePath=MochiSciencePath.fresh();
  ...
  if(typeof scNew==='function'){SCI.mode='practice';SCI.assessment=null;scNew();}
...
async function boot(){
  ...
  await hydrate();
  if(window.MochiEntrance)S.entrance=MochiEntrance.validate(S.entrance);
  if(window.MochiSciencePath)S.sciencePath=MochiSciencePath.validate(S.sciencePath);
...
if('serviceWorker' in navigator){
  document.addEventListener('mochi:ready', ()=> navigator.serviceWorker.register('sw.js?v=6.2.1',{updateViaCache:'none'}).catch(()=>{}),{once:true});
}
