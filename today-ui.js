...
(function(root){
'use strict';
const T=root.MochiToday,$=id=>document.getElementById(id),names=T.names;
let home,bar,menu,guided=false,transition=false,lastHome='',queued=null,lastFocus=null;
...
function paint(){
 ...
 if(!home.hidden){
  const signature=JSON.stringify(m);if(signature!==lastHome){lastHome=signature;
   ...
   $('todayTitle').textContent=m.restDay&&!t?'A little room to rest.':m.finished&&!t?'That’s today’s plan.':'One step at a time, Euna.';
   $('todayIntro').textContent=isPaper?'Your saved paper comes first. Pick up where you left off.':m.restDay&&!t?'No study time is scheduled today. There is no catch-up debt.':m.finished&&!t?'Both your effort and your questions matter. Time goals are not a mastery score.':'Your next activity is ready. There’s no need to choose a topic.';
   $('todayNowLabel').textContent=t?(isPaper?'SAVED WORK':t.resume?'PICK UP HERE':'UP FIRST'):'ALL SET';
   $('todayTaskTitle').textContent=t?t.title:'A break with Mochi';
   $('todayTaskSubject').textContent=t?names[t.subject]:m.restDay?'Rest day':'Study time goals reached';
   $('todayTaskDetail').textContent=t?t.detail:'Your learning and saved work will be here when you return.';
   $('todayStart').textContent=t?(t.resume?'Continue ':'Start ')+names[t.subject]+(isPaper?' paper':''):'Visit Mochi';
   $('todayStart').dataset.subject=t?.subject||'';
   $('todaySteps').innerHTML=m.order.map((b,i)=>`<li class="today-step" data-status="${b.done?'done':b.rest?'rest':t?.subject===b.subject?'now':'next'}"><span class="today-step-mark" aria-hidden="true">${b.done?'✓':b.rest?'–':i+1}</span><span><strong>${esc(b.name)}</strong><small>${b.rest?'Rest day':b.done?'Time goal reached':isPaper?'Lesson time can wait':t?.subject===b.subject?'Your current focus':'Up next · '+esc(b.task.title)}</small></span><span class="today-minutes">${b.rest?'—':b.done?b.minutes+' min':Math.max(1,Math.ceil(b.remaining/60000))+' min'+(b.spent?' left':'')}</span></li>`).join('');
   $('todayProgress').textContent=m.restDay?'No scheduled study today':`${m.completed} of ${m.planned} study time goals reached`;
   $('todayProgressBar').max=m.planned||1;$('todayProgressBar').value=m.completed;
   $('todayNote').textContent=isPaper?'The original paper deadline still applies. The daily study clock is separate.':'This is time for reading and thinking, not a speed test. Reviews fit inside the plan.';
  }
...
function init(){
 ...
 home.innerHTML=`<div class="today-greeting"><img src="euna-avatar.webp" width="58" height="58" alt=""><p id="todayDate"></p></div><h1 id="todayTitle" tabindex="-1"></h1><p id="todayIntro" class="today-intro"></p><section class="today-card" aria-label="Today’s study plan"><div class="today-card-top"><span id="todayNowLabel" class="today-eyebrow"></span><span id="todayTaskSubject" class="today-subject"></span></div><h2 id="todayTaskTitle"></h2><p id="todayTaskDetail"></p><button id="todayStart" class="today-primary" type="button">Start today’s plan</button><ol id="todaySteps" aria-label="Today’s study order"></ol><div class="today-progress"><span id="todayProgress"></span><progress id="todayProgressBar" max="2" value="0" aria-label="Daily study time goals reached"></progress></div></section><p id="todayNote" class="today-note"></p><button id="todayMochi" class="today-mochi" type="button"><img id="todayMochiImage" src="mochi-flat-avatar.svg" width="44" height="44" alt=""><span>Mochi is here.<small>Visit the room</small></span><span aria-hidden="true">↗</span></button>`;
 ...
 document.addEventListener('mochi:state-saved',()=>{clearTimeout(queued);queued=setTimeout(paint,100);});
 ...
 const reset=$('resetBtn').onclick;$('resetBtn').onclick=()=>{const before=S.learning;reset();if(before!==S.learning)open();};
