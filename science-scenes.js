/* Original scientific diagrams. Raster nature plates are original commissioned app illustrations. */
(function(root){
'use strict';
const svg=(body,label)=>`<svg viewBox="0 0 600 350" role="img" aria-label="${label}" xmlns="http://www.w3.org/2000/svg"><defs><marker id="scArrow" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0 0L6 3L0 6" fill="none" stroke="currentColor" stroke-width="1.5"/></marker></defs><g font-family="system-ui,sans-serif" font-size="25" fill="#352452" stroke-linecap="round" stroke-linejoin="round">${body}</g></svg>`;
...
const details={
 ...
 body:{question:'How does a meal help a cell far from the stomach?',limit:'A simplified pathway diagram, not scale anatomy. It connects digestive, respiratory and circulatory functions without showing every organ.',challenge:'Trace food to absorbed nutrients, and air to oxygen in blood. Where do the two routes connect?'}
};
...
function figure(skill,s,shown){
 ...
 if(skill==='heat'){
  ...
  return svg(`${line(70,60,70,292,'#aa9cb9',2)}${line(70,292,550,292,'#aa9cb9',2)}${[20,40,60,80].map(t=>label(34,300-(t-20)*3.1,t)+line(70,292-(t-20)*3.1,550,292-(t-20)*3.1,'#e8e0f1',1)).join('')}<path d="${curve(.085)}" fill="none" stroke="#277e8a" stroke-width="5"/><path d="${curve(.025)}" fill="none" stroke="#874acf" stroke-width="5"/>${shown?line(x,75,x,292,'#544261',2):''}${label(130,34,'Temperature / °C')}${label(70,340,'0')}${label(510,340,'20 min')}${label(430,80,'Wrapped')}${label(445,247,'Bare')}`,'Illustrative temperature against time curves: a wrapped cup cools more slowly');
 }
 ...
 if(skill==='ecosystems'){
  const arrow=(x1,y1,x2,y2,active)=>`<path d="M${x1} ${y1}L${x2} ${y2}" fill="none" stroke="${active?'#8447c9':'#bdb0cd'}" stroke-width="${active?6:3}" marker-end="url(#scArrow)"/>`;
  return `<img class="sc-pond" src="science-pond.webp" width="1080" height="720" alt="Original pond habitat illustration with green plants, a snail, fish, a frog and a dragonfly">`+svg(`${arrow(145,260,270,110,s.organism==='algae')}${arrow(145,270,330,270,s.organism==='algae')}${arrow(320,95,465,165,s.organism==='snail')}${arrow(395,255,485,180,s.organism==='larvae')}${[['Algae',90,280],['Snail',290,75],['Larvae',385,300],['Fish',500,150]].map(([t,x,y])=>label(x,y,t)).join('')}`,'Possible energy pathways: algae to snail and insect larvae; snail and larvae to fish');
 }
 if(skill==='forces'){
  const m=MochiScience.force(s);
  return svg(`<rect x="218" y="150" width="164" height="88" rx="8" fill="#e5d4fd" stroke="#715196" stroke-width="3"/><circle cx="252" cy="250" r="15" fill="#715196"/><circle cx="348" cy="250" r="15" fill="#715196"/>${line(50,270,550,270,'#baadc9',4)}<path d="M385 185h${Math.max(1,m.push)*17}" stroke="#7742b5" stroke-width="8" opacity="${m.push?1:0}" marker-end="url(#scArrow)"/><path d="M215 217h-${Math.max(1,m.friction)*22}" stroke="#277e8a" stroke-width="8" opacity="${m.friction?1:0}" marker-end="url(#scArrow)"/>${label(456,144,`${m.push} N`)}${label(136,186,`${m.friction} N`)}${label(300,202,'2 kg')}${label(300,55,'Already moving right')}${label(300,331,shown?`Net force: ${Math.abs(m.net)} N ${m.net>0?'right':m.net<0?'left':'(balanced)'}`:'Compare the opposing forces')}`,'Force diagram of a cart already moving right, with push right and friction left');
 }
 if(skill==='matter'){
 ...
 const selected=s.system;
 const box=(x,y,w,t,active)=>`<rect x="${x}" y="${y}" width="${w}" height="52" rx="15" fill="${active?'#dfcafa':'#f3eef8'}" stroke="${active?'#8553b7':'#c8b8d8'}" stroke-width="3"/>${label(x+w/2,y+34,t)}`;
 return svg(`${box(35,30,215,'Mouth',selected==='digestive')}${box(350,30,215,'Air in lungs',selected==='respiratory')}${box(35,128,215,'Stomach',selected==='digestive')}${box(35,226,215,'Small intestine',selected==='digestive')}${box(350,128,215,'Blood',selected==='circulatory')}${box(350,226,215,'Body cells',selected==='circulatory')}${line(142,83,142,128)}${line(142,181,142,226)}${line(457,83,457,128)}${line(457,181,457,226)}<path d="M250 252L325 252L325 155L350 155" fill="none" stroke="#8356b7" stroke-width="4" marker-end="url(#scArrow)"/>${label(140,115,'Oesophagus')}${label(458,320,'Nutrients + oxygen')}`,'Schematic links between digestion, absorption, oxygen uptake, blood and body cells');
}
