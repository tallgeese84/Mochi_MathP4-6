...
test('science revision stays open and supported correct answers never become independent',()=>{
 ...
 h.run('SCI.probeChoice=MochiScience.probeFor(SCI.q).answer;SCI.choice=1;scCheck();');assert.equal(h.run('SCI.done'),false);assert.equal(h.run('SCI.tries'),1);
...
test('guesses and restored inconsistent choices cannot claim independent understanding',()=>{
 const h=studio();h.nodes.get('scExplanation').value='This is my tentative explanation.';h.nodes.get('scGuess').checked=true;h.run('SCI.probeChoice=MochiScience.probeFor(SCI.q).answer;SCI.choice=SCI.q.answer;scCheck();');assert.equal(h.run('scData().attempts.at(-1).independent'),false);
