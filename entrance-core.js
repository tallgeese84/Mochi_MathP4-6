/* Entrance preparation evidence is separate from ordinary P4–P6 practice.
   Persistence is additive. No historical learner answers are built into this code. */
(function(root){
'use strict';
...
function init(s){return s.entrance||(s.entrance=fresh());}
...
function startPractice(d,id,{phase,seed,now=Date.now(),force=false}={}){
 ...
 d.draft={id:'entrance-'+uid(),unit:id,form,seed:value,at:now,updatedAt:now,phase,helped:phase==='guided',revealed:false,guess:false,seenBefore,lessonViewedAt:l.lastViewedAt||0,recallOf:phase==='recall'?e.lastSuccess:'',answer:'',working:'',strokes:[]};return d.draft;
}
...
function paperQuestions(id){
 ...
 const list=def.units.map((unit,i)=>{const form=def.kind==='baseline'?1:3;let seed=parseInt(B.hash('ep-paper-v1:'+id+':'+unit),36)>>>0,q=B.make(unit,form,seed);for(let k=0;k<200&&earlier.has(q.fingerprint);k++)q=B.make(unit,form,seed=(seed+1)>>>0);if(earlier.has(q.fingerprint))throw Error('Paper family has no unseen variant');earlier.add(q.fingerprint);return q;});
