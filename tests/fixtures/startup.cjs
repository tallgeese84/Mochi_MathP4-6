...
(async()=>{
 ...
 const dock=w.document.getElementById('focusDock'),surface=w.document.getElementById('studioSurface');
 assert.equal(dock.parentElement,surface,'navigation shares the content flow on tablet/mobile');
 assert.ok(dock.compareDocumentPosition(w.document.getElementById('viewMaths')) & w.Node.DOCUMENT_POSITION_FOLLOWING,'toolbar precedes the question surface');
