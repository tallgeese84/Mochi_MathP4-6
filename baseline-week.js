/* Mochi v4.2.8 — learner-facing task funnel.
   Important: the home panel is state-aware. It does NOT rebuild on every timer tick,
   so illustration and motion layers can keep animating smoothly. */
(function(){
'use strict';
...
function taskState(l){
  ...
  if(!d.finished){
    const stage=stageFor(d.done),part=BASELINE_STAGES.indexOf(stage)+1,remaining=Math.max(1,stage.end-d.done);
    return{phase:'baseline',part,eyebrow:`STARTING-POINT CHECK · PART ${part} OF 4`,title:d.done?'Continue today’s starting-point check':'Start your maths starting-point check',subtitle:`${remaining} question${remaining===1?'':'s'} left · about ${Math.max(5,remaining*3)} min · no timer`,note:stage.short+'. '+stage.detail+'.',cta:d.done?'Continue today’s task':'Start today’s task',done:d.done,total:BASELINE_TOTAL};
  }
  ...
  if(dailyDone<8){
    ...
    return{phase:'transfer',eyebrow:'TODAY · ADAPTIVE TRANSFER CHECK',title:'Use what you know in new-looking problems',subtitle:`${remaining} question${remaining===1?'':'s'} left · about ${Math.max(8,remaining*3)} min`,note:'Mochi chooses fresh forms from the baseline evidence. Do not ask which method to use first.',cta:dailyDone?'Continue today’s task':'Start today’s task',done:dailyDone,total:8};
  }
  const s=l?.session;
  const finishedToday=!!(s&&s.finished&&sameDay(localDay(s.started),localDay()));
  if(finishedToday)return{phase:'done',eyebrow:'TODAY · COMPLETE',title:'You finished today’s Mochi work',subtitle:'Nice work. Save one useful idea, then stop.',note:'Tomorrow Mochi will choose the next review from your evidence.',cta:'',done:1,total:1};
...
function startTask(){
  const l=learning();if(!l)return;const t=taskState(l);
  if(t.phase==='baseline'){
...
function simplifyAdvanced(){
  ...
  const mode=document.getElementById('studyMode');if(mode){const parent=mode.closest('label')||mode.parentElement;if(parent)parent.classList.add('mochi-secondary-choice');}
  document.getElementById('studyStart')?.classList.add('mochi-secondary-choice');
  const labels=['think','explore','my map','euna’s session','euna\'s session','challenge','mochi’s room','mochi\'s room'];
  document.querySelectorAll('button').forEach(b=>{if(labels.includes((b.textContent||'').trim().toLowerCase()))b.classList.add('mochi-secondary-choice');});
}
function style(){
  if(document.getElementById('mochiFocusStyle'))return;
  const s=document.createElement('style');s.id='mochiFocusStyle';s.textContent=`
    #mochiFocusHome{margin:4px 0 16px;position:relative}
    ...
    .focus-progress{height:10px;background:var(--line);border-radius:99px;overflow:hidden;margin:12px 0 14px}.focus-progress>i{display:block;height:100%;background:var(--jade);border-radius:inherit;transition:width .3s ease}
    .focus-start{width:100%;font-size:1.08rem;padding:15px 20px}.focus-done{padding:12px 14px;border-radius:14px;background:var(--jade-soft);font-weight:800;color:var(--jade)}
    .focus-week{background:rgba(255,255,255,.72);border:2px solid var(--line);border-radius:20px;padding:16px 16px 12px;margin-bottom:10px}.focus-week-head{display:flex;align-items:end;justify-content:space-between;gap:12px;margin-bottom:10px}.focus-week h2{font-family:var(--disp);font-size:1.08rem;margin:0}.focus-week-head span{font-size:.75rem;color:var(--slate)}
    .focus-list{display:grid;gap:6px}.focus-row{position:relative;display:grid;grid-template-columns:26px 72px 1fr;gap:8px;align-items:start;padding:8px 5px;border-radius:12px}.focus-row.current{background:var(--ginger-soft)}.focus-row.done{opacity:.62}.focus-check{width:22px;height:22px;border:2px solid var(--line);border-radius:50%;display:grid;place-items:center;font-size:.75rem;font-weight:900}.focus-row.done .focus-check{background:var(--jade);border-color:var(--jade);color:white}.focus-row.current .focus-check{border-color:var(--ginger);color:var(--ginger)}
    ...
    .focus-more{margin:8px 0 14px}.focus-more>summary{cursor:pointer;text-align:center;color:var(--slate);font-weight:800;font-size:.86rem;padding:8px}.focus-more-note{font-size:.8rem;color:var(--slate);text-align:center;margin:2px 0 8px}
    body.mochi-focus-mode:not(.mochi-show-more) .tabs{display:none!important}body.mochi-focus-mode:not(.mochi-show-more) .mochi-secondary-choice{display:none!important}
...
function paint(force=false){
  ...
  host.innerHTML=`
    <section class="focus-today" data-phase="${t.phase}" data-part="${t.part||''}">
      <p class="focus-kicker">${t.eyebrow}</p><p class="focus-date">${fullToday()}</p><h1 class="focus-title">${t.title}</h1><p class="focus-sub">${t.subtitle}</p><p class="focus-note">${t.note}</p>
